...
from . import _MODEL_PATH

def set_args():
    parser = argparse.ArgumentParser(description='Optimize mRNA using Codon BERT')
    parser.add_argument('-m', '--model', help='the model path', type=str,
                        default = _MODEL_PATH)
    parser.add_argument('-i', '--input', help='the seq fasta (amino acids)',required = True, type = str)             
    parser.add_argument('-o', '--output', help='the save path',required = True, type = str)
...
def main():
    ...
    seq_file = args.input
    model_output_save_path = args.output
    model_output_fix_save_path = args.output.split('.')[0]+"_fix.fasta"
